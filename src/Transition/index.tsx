...
export type TransitionVariantCallback = <T extends Record<string, number>>(
  el: HTMLElement,
  rect: DOMRect,
) => {
  from: T;
...
export type TransitionVariantTypes =
  | keyof typeof variants
  | TransitionVariantCallback;
...
function TransitionBase({ children, ...options }: TransitionProps) {
  ...
  const trans = useTransition({
    ...
    onEntered: () => {
      const v: any = variantCb(ref.current as HTMLElement, rect.current as any);
      if (v.onEntered) {
        v.onEntered();
      }
      onEntered && onEntered();
...
const Transition = (props: any) => {
  return <TransitionBase key={props.variant} {...props} />;
};
