...
export type useBreakpointPropsType<P> =
  | P
  | {
      [key in BreakpointKeys]?: P;
    };

const useBreakpointProps = <P extends object>(
  props: useBreakpointPropsType<P>,
): useBreakpointPropsType<P> => {
  ...
  const bprops = useMemo(() => {
    ...
    for (let prop in props) {
      ...
      if (
        ...
        typeof val === "object" &&
        val !== null &&
        Object.keys(val).some((k) => bkeys.includes(k))
      ) {
        bprops.push(prop);
      }
    }
